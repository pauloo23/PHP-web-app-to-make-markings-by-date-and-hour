...
SELECT ADDTIME(requester_hour,SEC_TO_TIME(requester_duration*60)), request_id,request_info,request_desc,requester_name,requester_hour,requester_mobile,request_state,assign_date FROM assignwork_tb
  WHERE requester_name LIKE '%" . $searchwork . "%'  AND request_state='0'
  OR requester_mobile LIKE '%" . $searchwork . "%' AND request_state='0'
  OR request_info LIKE '%" . $searchwork . "%' AND request_state='0'
  OR requester_name LIKE '%" . $upper . "%'AND request_state='0'
  OR request_info LIKE '%" . $upper . "%' AND request_state='0'
  OR assign_date LIKE '%" . $upper . "%' AND request_state='0'
OR assign_date LIKE '%" . $searchwork . "%' AND request_state='0'
  ORDER BY assign_date,
  requester_hour
 ";
} else {
    $query = "SELECT ADDTIME(requester_hour,SEC_TO_TIME(requester_duration*60)), request_id,request_info,request_desc,requester_name,requester_hour,requester_mobile,request_state,assign_date FROM assignwork_tb WHERE request_state=0   ORDER BY assign_date, requester_hour
 ";
}
...
if (mysqli_num_rows($resultwork) > 0) {
 ...
 <table class="content-table">   <thead>
    <tr>
     ...
     <th>Hora Inicio</th>
     <th>Hora Fim</th>
     <th style="overflow-x:auto;">Ação</th>         <th></th>
          <th></th>
    </tr>      </thead>
 ...
 ';
   ...
   <tr>
    ...
    <td>' . $row["assign_date"] . '</td>
     ...
     <td><form action="viewassignwork.php" method="POST" class="d-inline"> <input type="hidden" name="id" value=' . $row["request_id"] . '><button type="submit" class="btn btn-warning" name="view" value="View"><i class="far fa-eye"></i></button></form></td>
